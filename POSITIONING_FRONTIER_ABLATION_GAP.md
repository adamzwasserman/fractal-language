...
3. **The benchmark may not measure what it is taken to measure.** Even the target of the optimization is unvalidated. The programme's own BabyLM 2026 work found that standard child-scale benchmark scores responded principally to lexical-distributional properties of the tokenizer and the answer template, with task competence a secondary signal at best, and it reported several apparent results as artifacts on those grounds (BLiMP supplement at 34 percent, EWoK at chance under four interventions). A system tuned to maximize such a score may be maximizing something other than the capability the score is taken to name. The cone-of-light discipline applies to the measuring stick as much as to the model, and this is a finding the programme reported against its own results, not a charge levelled only at others.

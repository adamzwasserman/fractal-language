...
def compute_corpus_h(lang, max_chars=5000000):
    ...
    h = compute_hurst_dfa(word_freq_series)
    return {"H_word_freq": h, "n_words": len(words), "vocab_size": len(word_counts)}
...
def compute_tokenized_h(lang, max_tokens=1000000):
    ...
    h = compute_hurst_dfa(token_freq_series)
    return {"H_token_freq": h, "n_tokens": len(all_tokens), "vocab_used": len(token_counts)}
...
if __name__ == "__main__":
    ...
    for lang in LANGUAGES:
        ...
        if r:
            ...
            h = r["H_word_freq"]
            print(f"H_word_freq={h:.3f}" if not np.isnan(h) else "nan")
        else:
            print("no data")
    ...
    for lang in LANGUAGES:
        ...
        if r:
            ...
            h = r["H_token_freq"]
            print(f"H_token_freq={h:.3f}" if not np.isnan(h) else "nan")
        else:
            print("no data")

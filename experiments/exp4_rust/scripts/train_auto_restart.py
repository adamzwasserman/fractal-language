...
This script continuously runs the training, restarting it when it stops due to memory limits
"""
import subprocess
import time
import sys
...
def run_training_session(lang, model_size, mode):
    ...
    script_path = Path(__file__).parent / "train_resumable.py"
    ...
    log_message(f"Starting training: {' '.join(cmd[1:])}")
...
def main():
    ...
    log_message(f"Starting auto-restart training: {lang} {model_size} {mode}")
    ...
    try:
        while True:
            ...
            # Parse output to get final step count
            lines = stdout.strip().split('\n')
            for line in reversed(lines):
                try:
                    # Look for "Memory limit reached at step X" first (most reliable)
                    if "Memory limit reached at step" in line:
                        step_part = line.split("step")[1].strip()
                        if step_part.isdigit():
                            total_steps = int(step_part)
                            log_message(f"Found step from 'Memory limit reached': {total_steps}")
                            break
                    # Look for "Stopping at step X"
                    elif "Stopping at step" in line:
                        step_part = line.split("step")[1].strip()
                        if step_part.isdigit():
                            total_steps = int(step_part)
                            log_message(f"Found step from 'Stopping at step': {total_steps}")
                            break
                    # Parse progress bar format like "15100/200000"
                    elif "|" in line and "/200000" in line:
                        # Look for pattern like "| 15100/200000"
                        parts = line.split("|")
                        for part in parts:
                            if "/200000" in part:
                                ...
                                if step_part.isdigit():
                                    total_steps = int(step_part)
                                    log_message(f"Found step from progress bar: {total_steps}")
                                    break
                        if total_steps > 0:
                            break
                    # Look for "Starting from step X"
                    elif "Starting from step" in line:
                        step_part = line.split("step")[1].strip()
                        if step_part.isdigit():
                            total_steps = int(step_part)
                            log_message(f"Found step from 'Starting from step': {total_steps}")
                            break
                except Exception as e:
                    log_message(f"Debug: Failed to parse step from line '{line}': {e}")
            
            log_message(f"Session {session_count} completed")
            log_message(f"Exit code: {exit_code}")
            log_message(f"Total steps reached: {total_steps}")
            
            # Check exit conditions
            if exit_code == -1:  # User interrupt
                log_message("Training stopped by user")
                break
            elif exit_code == 0:  # Normal completion
                # Only consider complete if we actually reached 200k steps or explicit completion
                if total_steps >= 200000 or "reached target steps" in stdout:
                    log_message("Training reached target steps - COMPLETE!")
                    break
                else:
                    log_message("Session completed (likely due to memory limit), restarting...")
            else:
    ...
    log_message("Auto-restart training finished")

#!/usr/bin/env python
"""
DDP Interleaved EN/FR training for transfer experiment.
Trains ONE model on alternating EN/FR chunks using 2 GPUs.
Pre-registered: OSF 10.17605/OSF.IO/SJ48B

Launch with: torchrun --nproc_per_node=2 train_interleaved_ddp.py 125M 200000 8
"""
import torch
...
BATCH_SIZE_PER_GPU = int(sys.argv[3]) if len(sys.argv) > 3 else 8  # Per GPU batch size
...
def run_grammar_probes(model, tokenizer, lang, step, device):
    ...
    results = [evaluate_probe(actual_model, tokenizer, p, device) for p in GRAMMAR_PROBES[lang]]
...
def log_training_step(step, loss, ppl, base_path, world_size, batch_per_gpu):
    log_dir = base_path / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
...
def create_interleaved_stream(chunk_dir, seq_len, seed, rank, world_size):
    """Create interleaved EN/FR data stream with DDP sharding."""
        ...
        while True:
            ...
            for chunk_idx in chunk_order:
                ...
                for i in range(0, len(tokens) - seq_len - 1, seq_len // 2):
                    # DDP sharding: each rank gets different sequences
...
def save_checkpoint(step, model, optimizer, loss, base_path, model_size, world_size, batch_per_gpu):
    ...
    meta = {
        ...
        "total_batch_size": batch_per_gpu * world_size,
        "experiment": "interleaved_transfer_ddp",
    ...
    actual_model = model.module if hasattr(model, 'module') else model
    state = {k: v.clone().cpu() for k, v in actual_model.state_dict().items()}
    save_file(state, str(model_path))
    torch.save(optimizer.state_dict(), str(optimizer_path))
    print(f"Checkpoint saved: step {step}")


def load_checkpoint(model, optimizer, base_path, model_size, device):
    checkpoint_dir = get_checkpoint_dir(base_path, model_size)
    ...
    if optimizer_path.exists():
        optimizer.load_state_dict(torch.load(str(optimizer_path), map_location=device, weights_only=True))
...
def main():
    ...
    if is_main_process(rank):
        print(f"=== DDP Interleaved Training {MODEL_SIZE} EN+FR ===")
        print(f"World size: {world_size} GPUs")
        print(f"Batch per GPU: {BATCH_SIZE_PER_GPU}, Total batch: {total_batch}")
        print(f"Target: {TARGET_STEPS:,} steps")
        print(f"Seed: {RANDOM_SEED}")
        print(f"Tokens per step: {total_batch * SEQ:,}")
    ...
    # Load checkpoint if exists (only main process, then broadcast)
    start_step = load_checkpoint(model, optimizer, BASE_PATH, MODEL_SIZE, device) + 1
    
    # Sync start_step across all processes
    ...
    # Create interleaved data stream (sharded by rank)
    stream = create_interleaved_stream(CHUNK_DIR, SEQ, RANDOM_SEED, rank, world_size)

    # Signal handling (main process only)
    ...
    if is_main_process(rank):
        pbar = tqdm(range(start_step, TARGET_STEPS + 1), desc="DDP Interleaved", 
                    initial=start_step - 1, total=TARGET_STEPS)
    else:
        pbar = range(start_step, TARGET_STEPS + 1)
    ...
    for step in pbar:
        # Check shutdown across all processes
        shutdown_tensor = torch.tensor([1 if shutdown_requested[0] else 0], device=device)
        dist.all_reduce(shutdown_tensor, op=dist.ReduceOp.MAX)
        if shutdown_tensor.item() > 0:
            if is_main_process(rank):
                avg_loss = running_loss / log_interval if running_loss > 0 else 0
                save_checkpoint(step - 1, model, optimizer, avg_loss, BASE_PATH, MODEL_SIZE, world_size, BATCH_SIZE_PER_GPU)
            break

        # Prepare batch (each GPU gets BATCH_SIZE_PER_GPU sequences)
        batch_x, batch_y = [], []
        ...
        avg_loss_step = loss_tensor.item() / world_size
        running_loss += avg_loss_step

        if step % log_interval == 0 and is_main_process(rank):
            avg_loss = running_loss / log_interval
            ppl = np.exp(avg_loss)
            log_training_step(step, avg_loss, ppl, BASE_PATH, world_size, BATCH_SIZE_PER_GPU)
        ...
        if step % checkpoint_interval == 0:
            ...
            if is_main_process(rank):
                save_checkpoint(step, model, optimizer, loss.item(), BASE_PATH, MODEL_SIZE, world_size, BATCH_SIZE_PER_GPU)

...
Trains EN and FR models simultaneously on single GPU with equal progress.
Includes comprehensive grammar and emergence probes at each checkpoint.
"""
import torch
...
BATCH_PER_LANG = int(sys.argv[3]) if len(sys.argv) > 3 else 9
...
print(f"Config: {MODEL_SIZE} DUAL (EN+FR), batch={BATCH_PER_LANG}/lang, seq={SEQ}, vocab={VOCAB}")
...
def log_grammar_results(results, lang, base_path):
    ...
    # Detailed per-probe log (for deep analysis)
...
def log_training_step(step, en_loss, en_ppl, fr_loss, fr_ppl, base_path):
    """Log both languages in a single combined log for easy comparison."""
    log_dir = base_path / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    ...
    with open(log_file, 'a', newline='') as f:
        ...
        writer.writerow([
            ...
            f"{fr_loss:.4f}",
            f"{fr_ppl:.2f}",
            f"{en_loss - fr_loss:.4f}",
            datetime.now().isoformat()
        ])

    # Individual logs (for compatibility)
...
# ==================== DATA ====================
def create_data_stream(lang, chunk_dir, seq_len, seed):
    """Create a data stream for a specific language."""
...
# ==================== TRAINING ====================
def main():
    print(f"=== Dual Training {MODEL_SIZE} EN+FR ===")
    ...
    if N_GPUS > 1:
        model_en = torch.nn.DataParallel(model_en)
        model_fr = torch.nn.DataParallel(model_fr)
        print(f"Using DataParallel across {N_GPUS} GPUs for both models")
    ...
    # Start from the minimum of the two (to keep them synchronized)
    start_step = min(step_en, step_fr) + 1
    if step_en != step_fr:
        print(f"WARNING: EN at step {step_en}, FR at step {step_fr}. Starting from {start_step - 1}")
        # If one is ahead, we'll just let it re-train those steps (safer than skipping)
    ...
    # Create data streams
    stream_en = create_data_stream("en", CHUNK_DIR, SEQ, RANDOM_SEED)
    stream_fr = create_data_stream("fr", CHUNK_DIR, SEQ, RANDOM_SEED)
    ...
    pbar = tqdm(range(start_step, TARGET_STEPS + 1), desc="Dual Training", initial=start_step - 1, total=TARGET_STEPS)
    ...
    for step in pbar:
        ...
        # ===== Train EN =====
        batch_x_en, batch_y_en = [], []
        for _ in range(BATCH_PER_LANG):
            seq = next(stream_en)
            batch_x_en.append(seq[:-1])
            batch_y_en.append(seq[1:])

        x_en = torch.tensor(np.array(batch_x_en), dtype=torch.long, device=DEVICE)
        y_en = torch.tensor(np.array(batch_y_en), dtype=torch.long, device=DEVICE)
        ...
        # ===== Train FR =====
        batch_x_fr, batch_y_fr = [], []
        for _ in range(BATCH_PER_LANG):
            seq = next(stream_fr)
            batch_x_fr.append(seq[:-1])
            batch_y_fr.append(seq[1:])

        x_fr = torch.tensor(np.array(batch_x_fr), dtype=torch.long, device=DEVICE)
        y_fr = torch.tensor(np.array(batch_y_fr), dtype=torch.long, device=DEVICE)
